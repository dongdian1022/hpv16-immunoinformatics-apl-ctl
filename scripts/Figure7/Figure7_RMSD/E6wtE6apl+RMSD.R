...
# (极简纯净版：统计口径为 20-50ns，无背景阴影，修复导出报错)
# ==========================================
...
# 2. 读取文件 (无视原本错误的第一列)
wt_data <- read.table("E6wt_10mer_rmsd.dat", col.names = c("Fake_Time", "RMSD"))
apl_data <- read.table("E6apl_10mer_rmsd.dat", col.names = c("Fake_Time", "RMSD"))
...
# 打上分组标签并合并数据集
wt_data$Group <- "WT (E6wt)"
apl_data$Group <- "APL (E6apl)"
...
# 5. 导出多格式高清晰度图片 (统一为 8.5 x 6 尺寸，保证系列图表大小完全一致)
out <- "Figure_RMSD_50ns_E6wt_E6apl_v1"

# 规避 cairo 报错，使用系统默认的矢量设备
ggsave(paste0(out, ".pdf"), plot = p, width = 8.5, height = 6)

# 生成 600 dpi 像素图用于日常检查或 PPT 展示
ggsave(paste0(out, ".png"), plot = p, width = 8.5, height = 6, dpi = 600)

# 生成无损压缩 TIFF 用于期刊最终提交

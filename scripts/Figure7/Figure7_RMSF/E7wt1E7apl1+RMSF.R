# ==========================================
# 结构免疫学 MD 数据可视化：多肽 RMSF 柔性对比
# (极简纯净版：完美对齐 20-50ns 数据与 RMSD 视觉风格)
# ==========================================

library(ggplot2)
library(dplyr)

# 1. 设置工作目录 (⚠️ 指向刚刚新建的 rmsf_v3 文件夹)
setwd("E:/HPV-T非临床文章/0.文章投稿+确定版本/Vaccines/三维结构图/HLA-A0201/图表制作/analysis/rmsf/rmsf_v3")

# 2. 读取文件 (⚠️ 确保读取的是刚刚重算的 20-50ns 纯净数据)
wt_data <- read.table("E7wt1_9mer_rmsf_20_50ns_v3.dat", col.names = c("Residue", "RMSF"))
apl_data <- read.table("E7apl1_9mer_rmsf_20_50ns_v3.dat", col.names = c("Residue", "RMSF"))

# 3. 数据预处理与位点对齐 (分配 P1-P9)
...
# 4. 设定配色方案 (严格与 RMSD 图保持一致)
custom_colors <- c("WT (E7wt1)" = "#5BA3D0", "APL (E7apl1)" = "#E08A3C")

# --- 锚位与改造位点标注参数 ---
anchor_pos <- c(2, 9)         # HLA-A*02:01 锚位 P2 与 C 端 PΩ
sub_pos    <- c(1)            # 改造位点
sub_lab    <- c("L1Y")
...
p <- ggplot(merged_data, aes(x = Position, y = RMSF, color = Group, group = Group)) +
  ...
  # 绘制点与线 (更新为 linewidth，符合最新 ggplot2 规范)
  geom_line(linewidth = 1.2) +
  geom_point(size = 3.5, shape = 16) + 
  
  # 🚨 魔法 1：复刻 RMSD 的完美 L 型黑线
  annotate("path", x = c(0.5, 0.5, 9.5), y = c(3, 0, 0), color = "black", linewidth = 1.2, linejoin = "mitre") +
  
  theme_classic(base_size = 15) +
  scale_color_manual(values = custom_colors) +
  
  # 坐标轴标签 (去除 caption，保持画面绝对极简，说明文字放进 Word 图注里)
  labs(x = "Peptide Position", y = "Backbone RMSF (Å)") +
  
  # 🚨 魔法 2：严格控制 X 轴与 Y 轴的起始点，消除多余留白
  scale_x_continuous(breaks = 1:9, labels = paste0("P", 1:9), expand = c(0, 0)) +
  scale_y_continuous(breaks = seq(0, 3, by = 0.5), expand = c(0, 0)) +
  coord_cartesian(xlim = c(0.5, 9.5), ylim = c(0, 3), clip = "off") + 
...
# 6. 统一格式与尺寸导出
out <- "Figure_RMSF_20_50ns_E7wt1_E7apl1_v1"

...
wt_raw <- read.table("E7wt2_10mer_hbond_avg_20_50ns.dat", header = TRUE, comment.char = "", check.names = FALSE)
apl_raw <- read.table("E7apl2_10mer_hbond_avg_20_50ns.dat", header = TRUE, comment.char = "", check.names = FALSE)
...
# 赋予分组标签
wt_raw$Group <- "WT (E7wt2)"
apl_raw$Group <- "APL (E7apl2)"
...
final_plot_data <- clean_data %>%
  ...
  filter(Occupancy >= 10) %>%
  mutate(Group = factor(Group, levels = c("WT (E7wt2)", "APL (E7apl2)"))) %>%
  # 强制补齐 0 值，确保红灰柱子同等纤细
...
write.csv(final_plot_data, "Hbond_Final_E7WT2_E7APL2.csv", row.names = FALSE)
...
print(p)
out <- "../hbond-v2/H-bond-E7wt2-E7apl2-v5"

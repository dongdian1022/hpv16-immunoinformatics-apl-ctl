...
# 针对 10-mer 体系绝对编号：P1(376), P2(377), P10(385)
# 20-50 ns 分析窗口结果
# ==========================================
...
wt_raw <- read.table("E6wt_10mer_hbond_avg_20_50ns.dat", header = TRUE, comment.char = "", check.names = FALSE)
apl_raw <- read.table("E6apl_10mer_hbond_avg_20_50ns.dat", header = TRUE, comment.char = "", check.names = FALSE)
...
# 赋予分组标签
wt_raw$Group <- "WT (E6wt)"
apl_raw$Group <- "APL (E6apl)"
...
clean_data <- all_raw %>%
  filter(
    grepl(paste0("_", target_p1, "@"), Acceptor) | grepl(paste0("_", target_p1, "@"), Donor) |
      ...
      grepl(paste0("_", target_p10, "@"), Acceptor) | grepl(paste0("_", target_p10, "@"), Donor)
  ) %>%
  rowwise() %>%
  mutate(
    ...
    Peptide_Pos = case_when(
      ...
      grepl(paste0("_", target_p10, "$"), Acc_Res) | grepl(paste0("_", target_p10, "$"), Don_Res) ~ "P10",
      TRUE ~ "Unknown"
    ),
    
    Other_Res = ifelse(grepl(paste0("_(", target_p1, "|", target_p2, "|", target_p10, ")$"), Acc_Res), Don_Res, Acc_Res),
...
final_plot_data <- clean_data %>%
  ...
  filter(Occupancy >= 10) %>%
  mutate(Group = factor(Group, levels = c("WT (E6wt)", "APL (E6apl)"))) %>%
  # 强制补齐 0 值，确保红灰柱子同等纤细
  complete(Group, nesting(Peptide_Pos, Plot_Label), fill = list(Occupancy = 0)) %>%
  mutate(Peptide_Pos = factor(Peptide_Pos, levels = c("P1", "P2", "P10"))) %>%
...
write.csv(final_plot_data, "Hbond_Final_E6WT_E6APL.csv", row.names = FALSE)
...
custom_colors <- c("WT (E6wt)" = "#1A4F8A", "APL (E6apl)" = "#8B3A0F")
...
print(p)
out <- "../hbond-v2/H-bond-E6wt-E6apl-v2"
